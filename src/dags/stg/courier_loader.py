...
class CourierRepository:
    ...
    def insert_entry(self, conn: Connection, obj) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
                    INSERT INTO stg.cs_couriers(object_id, object_value, update_ts)
    ...
    def load_couriers(self):
        ...
        with self.pg_dest.connection() as conn:
            ...
            while True:
                ...
                load_queue = self.origin.get_couriers_from_api(limit, offset)

                self.log.info(f"Reveived {i} batch of {len(load_queue)} couriers to load.")
                if not load_queue:
